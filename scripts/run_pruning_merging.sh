...
config_name=pruning_merging.yaml
...
CUDA_VISIBLE_DEVICES=$free_gpu fairseq-hydra-train \
    ...
    task.tokenizer_bpe_model=${tokenizer_ckpt} \
    model.teacher_path=${teacher_ckpt} \
    model.student_path=${exp_path}/checkpoints/merged_pruned_checkpoint_last.pt \
    distributed_training.distributed_world_size=${gpus} \
    distributed_training.nprocs_per_node=${gpus} \
    optimization.update_freq=[${update_freq}] \
    dataset.num_workers=${workers} \
...
python ../avhubert/save_final_ckpt.py \
    ...
    --original_ckpt ${exp_path}/checkpoints/merged_pruned_checkpoint_last.pt  \
...
CUDA_VISIBLE_DEVICES=$free_gpu fairseq-hydra-train \
    ...
    ${finetune_noise_opts} \
    task.tokenizer_bpe_model=${tokenizer_ckpt} \
    model.w2v_path=${exp_path}/final/checkpoints/merged_pruned_checkpoint_last_final.pt \
    distributed_training.distributed_world_size=${gpus} \
    distributed_training.nprocs_per_node=${gpus} \
