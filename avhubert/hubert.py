...
class AVHubertConfig(FairseqDataclass):
    ...
    encoder_ffn_embed_dim_detailed: List[int] = field(
        default_factory=lambda: [],
        metadata={"help": "FFN embedding dimension per encoder layer"}
    )
    encoder_use_ffn: List[bool] = field(
        default_factory=lambda: [],
        metadata={"help": "use FFN in encoder layer"},
    )
    encoder_attention_heads: int = field(
        default=12, metadata={"help": "num encoder attention heads"}
    )
    encoder_attention_heads_detailed: List[int] = field(
        default_factory=lambda: [],
        metadata={"help": "number of attention heads per encoder layer"}
    )
    encoder_attention_heads_dim: int = field(
        default=64,
        metadata={"help": "encoder attention head dimension"},
    )
    encoder_use_attention: List[bool] = field(
        default_factory=lambda: [],
        metadata={"help": "use attention in encoder layer"},
    )
    ...
    resnet_layers_detailed: List[Any] = field(
        default_factory=lambda: [],
        metadata={"help": ""},
    )
    ...
    # Pruning
    resnet_prune_conv_channels: bool = field(
        default=False,
        metadata={"help": "prune resnet convolutional channels"},
    )
    encoder_prune_attention_heads: bool = field(
        default=False,
        metadata={"help": "prune encoder attention heads"},
    )
    encoder_prune_attention_layer: bool = field(
        default=False,
        metadata={"help": "prune encoder attention layer"},
    )
    encoder_prune_ffn_intermediate: bool = field(
        default=False,
        metadata={"help": "prune encoder FFN intermediate dimension"},
    )
    encoder_prune_ffn_layer: bool = field(
        default=False,
        metadata={"help": "prune encoder FFN layer"},
    )
    encoder_merge_type: Optional[str] = field(
        default=None,
        metadata={"help": "specify the merged weight matrices in an attention head. Choose from [QKV, QK, QV, KV]"},
    )
...
class SubModel(nn.Module):
    ...
    def prune(self):
        out_channel_index, resnet_config = self.resnet.prune()
        if out_channel_index is not None:
            prune_linear_layer(self.proj, out_channel_index, "input")
        return resnet_config
    
    def get_num_params(self):
        num_params = 0
        if self.resnet is not None:
            resnet_num_params, out_channels = self.resnet.get_num_params_and_out_channels()
            num_params += resnet_num_params
        else:
            out_channels = self.proj.in_features
        num_params += (out_channels + 1) * self.proj.out_features
        if self.encoder is not None:
            num_params += self.encoder.get_num_params()
        return num_params
...
class AVHubertModel(BaseFairseqModel):
    def __init__(
        ...
        **kwargs
        ...
        resnet = ResEncoder(relu_type=cfg.resnet_relu_type, weights=cfg.resnet_weights, layer_detailed=cfg.resnet_layers_detailed ,prune_conv_channels=cfg.resnet_prune_conv_channels)
    ...
    def extract_intermediate_features(self, source: torch.Tensor, target_list: Optional[List[torch.Tensor]] = None, padding_mask: Optional[torch.Tensor] = None, mask=False) -> List[torch.Tensor]:
        src_audio, src_video = source['audio'], source['video']
        if mask and self.masking_type == 'input':
            src_video, mask_indices_video = self.apply_input_mask(src_video, padding_mask, target_list)
            src_audio, mask_indices_audio = self.apply_input_mask(src_audio, padding_mask, target_list)
        ...
        # padding_mask: (B, T), bool
        # mask_indices: (B, T), bool
        x = self.encoder.get_intermediate_outputs(x, padding_mask=padding_mask)
    ...
    def merge(self, t=0.01, type="QKV"):
        self.encoder.merge(t, type)
    
    def prune(self):
        self.eval()     # must be in eval mode
        resnet_config = self.feature_extractor_video.prune()
        encoder_config = self.encoder.prune()
        encoder_use_attention = encoder_config["encoder_use_attention"]
        encoder_use_fnn = encoder_config["encoder_use_ffn"]
        encoder_attention_heads = encoder_config["encoder_attention_heads"]
        encoder_fnn_embed_dim = encoder_config["encoder_ffn_embed_dim"]

        return resnet_config, encoder_use_attention, encoder_use_fnn, encoder_attention_heads, encoder_fnn_embed_dim
    
    def get_num_params(self, component: Optional[str] = None):
        if component is not None:
            assert component in ["feature", "encoder"]
        num_params = 0
        if component is None or component == "feature":
            num_params += self.feature_extractor_audio.get_num_params()
            num_params += self.feature_extractor_video.get_num_params()
            num_params += self.layer_norm.normalized_shape[0] * 2
            if self.post_extract_proj is not None:
                num_params += (self.post_extract_proj.in_features + 1) * self.post_extract_proj.out_features
        if component is None or component == "encoder":
            num_params += self.encoder.get_num_params()
        if component is None:
            num_params += self.mask_emb.numel()
            if self.target_glu is not None:
                num_params += (self.target_glu[0].in_features + 1) * self.target_glu[0].out_features
            if self.final_proj is not None:
                num_params += (self.final_proj.in_features + 1) * self.final_proj.out_features
            num_params += self.label_embs_concat.numel()
        return num_params
